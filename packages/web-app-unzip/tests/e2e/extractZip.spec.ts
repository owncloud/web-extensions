...
test('extract zip file', async () => {
  ...
  await expect(adminPage.getByRole('button', { name: 'logo-wide.png' , exact:true })).toBeVisible()
  await expect(adminPage.getByRole('link', { name: 'lorem.txt' ,exact:true })).toBeVisible()
})
